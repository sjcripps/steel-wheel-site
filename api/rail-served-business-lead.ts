...
import { sendLeadEmail, syncWorkmateCrm } from './_lead-email';

// Rail-Served Business Directory lead capture. Pure email-gate (no compute
// payload): user submits email to unlock the directory, we capture the lead.
// Mirrors the transload-directory lead pattern.
//
// Sinks (best-effort, isolated try/catch on each):
//   1. S3 append at s3://openclawbucket/jakecbot/swl-leads/rail-served-directory.jsonl
//   2. Telegram DM to Jacob via Bot API
//   3. info@swl email (Resend)
//   4. Workmate CRM Supabase sync
//
// Synthetic test traffic (@anthropic.com) is short-circuited.

const S3_BUCKET = 'openclawbucket';
const S3_KEY = 'jakecbot/swl-leads/rail-served-directory.jsonl';
...
async function appendS3(record: Record<string, unknown>): Promise<boolean> {
  try {
    ...
    console.error('rail-served-business-lead S3 write failed:', err);
    return false;
  }
}
...
async function sendTelegram(message: string): Promise<boolean> {
  if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_USER_ID) {
    console.warn('rail-served-business-lead: Telegram env vars missing, skipping DM');
    return false;
  }
  try {
    ...
    if (!res.ok) {
      ...
      console.error('rail-served-business-lead Telegram non-200:', res.status, body);
      return false;
    }
    return true;
  } catch (err) {
    console.error('rail-served-business-lead Telegram fetch failed:', err);
    return false;
  }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const message =
    `\u{1F684} SWL rail-served-business-directory lead: ${email}${companyStr}. ${nowCstShort()}`;

  const emailPromise = sendLeadEmail({
    toolName: 'rail-served business directory',
    customerEmail: email,
    subject: `SWL Rail-Served Business Directory Lead — ${email}${company ? ' (' + company + ')' : ''}`,
    heading: 'New Rail-Served Business Directory Lead',
    rows: [
  ...
  const crmPromise = syncWorkmateCrm({
    ...
    name: leadName,
    phone: leadPhone,
    source: 'swl-rail-served-businesses',
    tags: ['directory', 'rail-served-businesses'],
    noteHeader: `[${nowCstShort()}] swl-rail-served-businesses`,
    noteLines: [
      `  Tool: rail-served business directory`,
